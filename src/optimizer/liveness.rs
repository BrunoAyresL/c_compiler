...
pub fn new_liveness_analyzer(instructions: Vec<Instruction>, blocks: Vec<Block>) -> LivenessAnalyzer {
    LivenessAnalyzer { inst_liveness: vec![InstructionLiveness {
        ...
        instructions, blocks, variables: HashMap::new() }
}
...
impl LivenessAnalyzer {
    ...
    pub fn gen_live_out(&mut self) {
        ...
        while changed {
            ...
            for i in (0..self.blocks.len()).rev() {
                ...
                if self.block_live_out(self.blocks[i].id) {
                    changed = true;
                }
            }
        }
    ...
    fn add_edge(&mut self, src: &String, dest: &String) {
        if src == dest { return; }
        self.variables.entry(src.to_string()).or_insert_with(|| 
            Variable { name:src.to_string(), edges: HashSet::new(), register_id: 0 })
            .edges
            .insert(dest.to_string());

        self.variables.entry(dest.to_string()).or_insert_with(|| 
            Variable { name:dest.to_string(), edges: HashSet::new(), register_id: 0 })
            .edges
